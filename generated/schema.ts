...
export class MarketItemEntity extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save MarketItemEntity entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save MarketItemEntity entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("MarketItemEntity", id.toString(), this);
    }
  }

  static load(id: string): MarketItemEntity | null {
    return changetype<MarketItemEntity | null>(
      store.get("MarketItemEntity", id)
    );
  }
